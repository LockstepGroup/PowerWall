function Get-PwSwNatPolicy {
	...
	:fileloop foreach ($entry in $LoopArray) {
        ...
        if ($KeepGoing) {
            ...
            # Enabled
            $EvalParams.Regex = [regex] "^Enabled\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams -ReturnGroupNum 1
            if ($Eval) {
                if ($Eval -eq 0) {
            ...
            # OriginalSource
            $EvalParams.Regex = [regex] "^Original\ Src\ Address\ Object\ +:\ +(.+)"
            ...
            # OriginalDestination
            $EvalParams.Regex = [regex] "^Original\ Dst\ Address\ Object\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams
            if ($Eval) {
            ...
            # TranslatedSource
            $EvalParams.Regex = [regex] "^Translated\ Src\ Address\ Object\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams
            if ($Eval) {
            ...
            $EvalParams.Regex = [regex] "^Translated\ Dst\ Address\ Object\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams
            if ($Eval) {
            ...
            # SourceInterface
            $EvalParams.Regex = [regex] "^Src\ Interface\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams
            if ($Eval) {
            ...
            $EvalParams.Regex = [regex] "^Dst\ Interface\ +:\ +(.+)"
            $Eval             = Get-RegexMatch @EvalParams
            if ($Eval) {
